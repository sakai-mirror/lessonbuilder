...
public class LessonBuilderEntityProducer extends AbstractEntityProvider 
	...
	private static final String ARCHIVE_VERSION = "2.5"; // in case new features
	// are added in
	// future exports
	...
	public void init() {
		...
		// this slightly odd code is for testing. It lets us test by reloading
		// just lesson builder.
		// otherwise we have to restart sakai, since the entity stuff can't be
		// restarted
		if (false) {
			try {
				...
				// we don't have an actual user at this point, so need to force
				// checks to work
	...
	// lessonbuilder allows new tools to be created that use lessonbuilder. They
	// will have
	...
	// tools. Note that the tools are loaded when LinkTool.class is loaded.
	// That's
	...
	protected void addPage(Document doc, Element element, SimplePage page) {
		...
		// we don't read this on input, as copying typically assumes you'll want
		// new release dates
		...
		try {
			...
			if (tools != null && !tools.isEmpty()) {
				for (ToolConfiguration config : tools) {
					...
					// addPage(doc, element,
					// simplePageToolDao.getTopLevelPageId(config.getPageId()));
		...
		for (int i = 0; i < length; i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (itemElement.getTagName().equals("item")) {
					...
					// URL is probably no longer used, but if it is, it probably
	...
	public String merge(String siteId, Element root, String archivePath, String fromSiteId, Map attachmentNames, Map userIdTrans, Set userListAllowImport) {
		...
		if (siteId != null && siteId.trim().length() > 0) {
			try {
				...
				// need to fill in the tool id for top level pages and set
				// parents to null
				...
				for (int i = 0; i < numTools; i++) {
					...
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						// there's an element at top level with no attributes.
						// ignore it
						...
						if (oldToolId != null) {
							...
							if (toolTitle != null) {
								...
								// some code in site action creates all the
								// pages and tools and some doesn't
								...
								if (toolConfs != null && !toolConfs.isEmpty()) {
									for (ToolConfiguration config : toolConfs) {
										if (config.getToolId().equals(LESSONBUILDER_ID)) {
											...
											// only use the Sakai page if it has
											// the right title
											// and we don't already have lessson
											// builder info for it
											if (p != null && toolTitle.equals(p.getTitle()) && simplePageToolDao.getTopLevelPageId(config.getPageId()) == null) {
												page = p;
												tool = config;
												break;
											}
								...
								// if we alrady have an appropriate blank page
								// from the template, page and tool are set
								...
								// now fix up the page. new format has it as
								// attribute
								...
								// fix up the new copy of the page to be top
								// level
								...
								// create the vestigial item for this top level
								// page
	...
	public final static String[] EVENT_KEYS = new String[] { "lessonbuilder.create", "lessonbuilder.delete", "lessonbuilder.update", "lessonbuilder.read" };

	/**
	 * Return an array of all the event keys which should be tracked for
	 * statistics
	 * 
	 * @return an array if event keys (example: "message.new" ,
	 *         "message.delete")
	 */
	 ...
	 * Return the event key => event name map for a given Locale, allows the
	 * author to create human readable i18n names for their event keys
	 * 
	 * @param locale
	 *            the locale to return the names for
	 * @return the map of event key => event name (example: for a 'en' locale:
	 *         {"message.new","A new message"}) OR null to use the event keys
	 */
